...
LHCOpticsApproximator::LHCOpticsApproximator(std::string name, std::string title, TMultiDimFit::EMDFPolyType polynom_type, std::string beam_direction, double nominal_beam_energy)
...
LHCOpticsApproximator::LHCOpticsApproximator(std::string name, std::string title, std::string beam_direction, double nominal_beam_energy,
                                            TMultiDimFit *x_parametrisation_object, TMultiDimFit *theta_x_parametrisation_object, 
                                            TMultiDimFit *y_parametrisation_object, TMultiDimFit *theta_y_parametrisation_object):
...
void LHCOpticsApproximator::Train(TTree *inp_tree, std::string data_prefix, polynomials_selection mode, int max_degree_x, int max_degree_tx, int max_degree_y, int max_degree_ty, bool common_terms, double *prec)
{
  ...
  for(int i=0; i<4; i++)
  {
    ...
    out_polynomials[i]->FindParameterization();
    std::cout<<"Out variable "<<coord_names[i]<<" polynomial"<<std::endl;
    out_polynomials[i]->Print("M");
    std::cout<<std::endl;
  }
