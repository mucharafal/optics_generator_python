...
def generate_particles_trajectory(configuration_path, configuration_file_name):
    ...
    # return it
    return (segments,target,ready_config_path)
...
def resupply(target,ready_config_path,segments):
    if np.size(segments)>=target: return segments
    else:
        run_madx(ready_config_path)
        s=read_in_madx_output_file("trackone")
        segments={**segments, **s}
        ressupply(target,ready_config_path,segments)
...
if __name__ == "__main__":
    ...
    (output,target,ready_config_path) = generate_particles_trajectory(config_path, file_config)

    output=ressupply(target,ready_config_path,output)
print(output)
