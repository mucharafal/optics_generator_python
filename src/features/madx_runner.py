...
def generate_particles_trajectory(configuration_path, configuration_file_name):
    ...
    for type in root.findall('item'):
        target = type.get('tot_entries_number')
...
def resupply(target, ready_config_path, segments, configuration, dir):
    ...
    global die
    while not die:
        if not os.path.exists(config_path+"/"+dir):
            os.mkdir(config_path+"/"+dir)
        work = os.getcwd()
    ...
    while True:
        for i in range(10):
            ...
            if os.path.isfile(path+"/trackone"):
                ...
                for key in segments:
                    if key in s:
                        segments[key] = np.concatenate((segments[key], s[key]), axis=0)
